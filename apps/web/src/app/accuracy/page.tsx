    ...
    <div className="flex flex-col gap-8">
      ...
      {overall.length === 0 ? (
        ...
        <>
          ...
          <section>
            <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-zinc-500">By league</h2>
            <div className="flex flex-col gap-4">
              {Array.from(byLeagueGrouped.entries()).map(([slug, stats]) => (
                <div key={slug} className="rounded-lg border border-zinc-200 p-4 dark:border-zinc-800">
                  <Link href={`/league/${slug}`} className="font-medium hover:underline">
                    {stats[0].league.name}
                  </Link>
